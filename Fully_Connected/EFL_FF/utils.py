...
def separate_data(data, num_clients, num_classes, niid=False, 
    ...
    # c = np.random.rand(N, M)-0.5
        ...
        R = s/s.max()/1.5
        cm_color = 'viridis'
    # R = s/s.max()/2 # if you want to scale the radius of the circles

    # fig, ax = plt.subplots(figsize=(4, 4)) #, facecolor='lightgrey')
    ...
    # ax.set(title='Number of samples per class per client', xlabel='Clients', ylabel='Classes')
...
def plot_block_assignment(pd_block_assignment, output_path, num_blocks=None, name=''):
    ...
    new_palette = [new_cmap(i) for i in np.linspace(0, 1, 8)]
